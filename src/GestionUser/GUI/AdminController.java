...
public class AdminController implements Initializable {
    ...
    public void fnshow(){
        ServiceUtilisateur sp=new ServiceUtilisateur();
         ObservableList<Utilisateur> list =FXCollections.observableList(sp.afficher()); 
     
     colNom.setCellValueFactory(new PropertyValueFactory<>("nomu"));
     colMail.setCellValueFactory(new PropertyValueFactory<>("mailu"));
     colMdp.setCellValueFactory(new PropertyValueFactory<>("mdp"));
     colNum.setCellValueFactory(new PropertyValueFactory<>("numerou"));
    ...
    private void fnSelectiannonces(MouseEvent event) {
         Utilisateur p = tab.getSelectionModel().getSelectedItem();
         lbidAnnonces.setText(p.getIdu()+"");
    ...
    private void fnModifier(ActionEvent event) {
       ...
       //ServiceCategorie sc=new ServiceCategorie();
        ...
        tfAdresse1.setText(p.getRole());
        tfNumero1.setText(p.getNumerou()+"");
        tfEmail1.setText(p.getMailu());
        tfNom1.setText(p.getNomu());
    }
...
p.setRole(tfAdresse1.getText());
p.setIdu(1);
p.setNumerou(Integer.parseInt(tfNumero1.getText()));
p.setMailu(tfEmail1.getText());
p.setNomu(tfNom1.getText());
ServiceUtilisateur sp = new ServiceUtilisateur();
sp.add(p);
fnshow();
pnMesAnnonces.toFront();
